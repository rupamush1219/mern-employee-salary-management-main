...
const DeductionData = () => {
    ...
    const filteredDeductionData = DeductionData.filter((SalaryDeduction) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                dispatch(deleteDeductionData(id)).then(() => {
                    Swal.fire({
                        title: 'Success',
                        text: 'Data deduction successfully deleted.',
                        icon: 'success',
                        timer: 1000,
                            ...
                            {filteredDeductionData.slice(startIndex, endIndex).map((data, index) => {
                                return (
                                    <tr key={data.id}>
                                        ...
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                                            <p className='text-black dark:text-white'>Rp. {data.total_deduction}</p>
                                        </td>
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                ...
                <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                    <div className="flex items-center space-x-2">
                        <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                            Showing {startIndex + 1}-{Math.min(endIndex, filteredDeductionData.length)} from {filteredDeductionData.length} Data Deduction
                        </span>
                    </div>
